...
//       -u        Source toggle interval in seconds.
...
* gMulticast = const_cast<char*>(MCASTADDRV4),   // Multicast group to join

* gPort = const_cast<char*>(MCASTPORT),          // Port number to use

** gSourceList = NULL;         // List of source IP addresses


BOOL toggleLastSource = FALSE;  // Validate if last toggle was true or false
...

{
    ...
    printf(" -u int Source toggle interval in seconds.\n");
...

{
...

    {
...

        {
            ...
            case 't':        // Size of send receive buffer
            ...
            case 'u':        // Source toggle interval in seconds

                if (i + 1 >= argc)

                    usage(argv[0]);

                toggleInterval = atol(argv[++i]);
...
int JoinMulticastGroup(SOCKET s, struct addrinfo* group, struct addrinfo* iface, BOOL toggle, BOOL drop)

{
...

    {
...

        {
...

            {
                ...
                ressrc = ResolveAddress(gSourceList[i], const_cast<char*>("0"), group->ai_family, group->ai_socktype, group->ai_protocol);
                ...
                if (toggle) {
                    if (i == (gSourceCount - 1)) {
                        if (drop) {
                            rc = setsockopt(s, IPPROTO_IP, IP_DROP_SOURCE_MEMBERSHIP, (char*)&mreqv4src, sizeof(mreqv4src));
                            updated = TRUE;
                        }
                        else
                        {
...

                {
                    if (updated) {
                        if (drop) {
                            //printf("%sDROP SOURCE%s: ", KRED, KNRM);
                            printf("DROP SOURCE: ");

                        }
                        else {
                            //printf("%sADD SOURCE%s: ", KGRN, KNRM);
                            printf("ADD SOURCE: ");
                        }
...

    {
        ...
        filter->imsf_fmode = MULTICAST_MODE_TYPE(gMode);
...
void toggleSourceTask(SOCKET ts, addrinfo* tresmulti, addrinfo* tresif, int tgInterval)
{
    if ((tgInterval != 0) && (gMode != MCAST_EXCLUDE)) {
        printf("toggleSourceTask started with : %d sec interval\n", tgInterval);
        int count = 0;
        while (!terminateThread) {
            std::this_thread::sleep_for(std::chrono::seconds(tgInterval));
            if (toggleLastSource) {
                JoinMulticastGroup(ts, tresmulti, tresif, true, false);
                toggleLastSource = FALSE;
            }
            else {
                JoinMulticastGroup(ts, tresmulti, tresif, true, true);
                toggleLastSource = TRUE;
            }

//            printf("toggleSourceTask count: %d\n", count);
            count++;
        }
    }
...

{
    ...
    resbind = ResolveAddress(gBindAddr, (bSender ? const_cast<char*>("0") : gPort), resmulti->ai_family, resmulti->ai_socktype, resmulti->ai_protocol);
    ...
    resif = ResolveAddress(gInterface, const_cast<char*>("0"), resmulti->ai_family, resmulti->ai_socktype, resmulti->ai_protocol);
...

    {
        ...
        rc = JoinMulticastGroup(s, resmulti, resif, false, false);
    ...
    // Thread for toggle add/drop sources
    
    std::thread t1(toggleSourceTask, s, resmulti, resif, toggleInterval);
